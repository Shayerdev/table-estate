...
class FormRecords extends HTMLElement {
    ...
    fieldRecordCity(){
        $(this).find('form-select-city select').on('select2:select',  (e) => {
    ...
    checkPriceAgree(){
        ...
        return new Promise((res, rej) => {
            document.querySelector('#decline-price-agree').addEventListener('click', () => {
                agreeModalPrice(false);
                rej('You decline saved data.');
            });
            document.querySelector('#accept-price-agree').addEventListener('click', () => {
                agreeModalPrice(false);
                res(true);
            });
        });
    ...
    saveRecords(){
        ...
        save_records.addEventListener('click', async () => {
            ...
            const checkPrice = document.querySelector('table-records .global-options .price-control input')
            try {
                ...
                if(!!checkPrice.value.length && checkPrice.value > 0) location.reload();
            } catch (e){
                this.errorAlert(e);
            }
        });
